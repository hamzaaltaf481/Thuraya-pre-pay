...
def signup_handler(session, s, mail):
    ...
    existing_user = (
        session.query(User).filter(User.email == email, User.role != "guest").first()
    )
    if existing_user:
    ...
    new_user = User(
        ...
        country_region=str(from_location),
        role="customer",
    )
    ...
    return (
        jsonify({"message": "Registered successfully. Please confirm your email!"}),
        200,
    )
