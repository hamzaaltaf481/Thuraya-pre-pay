...
def test_mss():
    with mss() as sct:
        ...
        screenshot = sct.grab(
            {"left": top_left[0], "top": top_left[1], "width": width, "height": height}
        )
...
def test_pillow():
    ...
    screenshot = ImageGrab.grab(
        bbox=(top_left[0], top_left[1], top_left[0] + width, top_left[1] + height)
    )
...
def test_parse():
    ...
    parent_td = soup.find(
        lambda tag: tag.name == "td" and "Activation date" in tag.text
    )
    response = {}

    status_td = parent_td.find(
        lambda tag: tag.name == "td" and "Current Status" in tag.text
    )
    if status_td:
    ...
    call_limit_td = parent_td.find(
        lambda tag: tag.name == "td" and "Outgoing call limit date" in tag.text
    )
    if call_limit_td:
    ...
    total_refill_td = parent_td.find(
        lambda tag: tag.name == "td" and "Total account refill" in tag.text
    )
    if total_refill_td:
    ...
    last_recharge_td = parent_td.find(
        lambda tag: tag.name == "td" and "Total account refill" in tag.text
    )
    if last_recharge_td:
...
def check_refill_status(card_number):
    ...
    second_td = first_td.find(
        lambda tag: tag.name == "td" and first_9_digits in tag.text
    )
    third_td = second_td.find(
        lambda tag: tag.name == "td" and first_9_digits in tag.text
    )
    fourth_td = third_td.find(
        lambda tag: tag.name == "td" and first_9_digits in tag.text
    )
...
def send_email(
    subject, message, from_addr, to_addr, smtp_server, smtp_port, username, password
):
    msg = MIMEMultipart()
    msg["From"] = from_addr
    msg["To"] = to_addr
